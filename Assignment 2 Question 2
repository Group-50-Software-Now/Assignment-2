#Group-50 Software Now Assignment-2 , Question 2
...
#This program is used to analyses temprature data from multiple years.
#Each CSV file has been used to represent one year of data.
#Also this program reads all CSV files in the tempratures folder,
...
# This function analyses temperature data to understand how consistent or 
#fluctuating each weather station’s readings are over time.

def temperature_stability(all_data):

# We use standard deviation because it is a reliable measure of variability. 
# A low value means temperatures stay fairly consistent, 
#while a high value indicates large fluctuations.
...
# Stations with the smallest standard deviation are considered the most stable because 
#their temperatures change very little.
